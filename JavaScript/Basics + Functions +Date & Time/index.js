...
for(let ch = 0; ch<str.length; ch++){
    if(str[ch]=='a' || str[ch]=='e' || str[ch]=='i' || str[ch]=='o' || str[ch]=='u') count++;
}
console.log(`count of vowels in ${str} is ${count}`);

//check whether a string contains all vowels
...
console.log(checkAllVowels(str1));


//check pangram string
const isPangram =(s)=>{
    let inputArr = s.toLowerCase().split("");

    const values = inputArr.filter((curr)=> curr.charCodeAt()>='a'.charCodeAt() && curr.charCodeAt()<='z'.charCodeAt());
